...
### 2026-08-04 — 大容量スキャンPDFのメモリ使用量削減

**事象**: 42ページ・約24.8MBのスキャンPDFで `Out of memory error` が複数回発生し、自動再試行後に成功した。

**対策**: PDF生成処理を独立スコープへ分離し、pdf-libの解析状態をアップロード処理へ持ち越さない構造に変更。保存直後のバイト数をアップロード処理へ引き渡し、サイズ確認用の `blob.getBytes()` を廃止した。さらに、resumable upload のPUTでは公式にサポートされるBlob直接payloadを使用し、アップロード用Byte[]の全量コピーを削減した。

ページごとの番号、描画位置、元ファイルIDへの上書き、番号再利用、Slack通知、メール添付の成果は変更していない。有限メモリのため任意サイズのPDF成功は保証できないが、今回の24.8MB級で重なっていた不要な全量コピーを除去した。
